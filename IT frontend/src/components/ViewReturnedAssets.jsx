...
//       ReturnedTo: item.returnTo || 'N/A',
//       ReturnReason: item.returnReason || 'N/A',
//       Remark: item.remark || 'N/A', // Exporting remark
//       IssuedDate: formatDate(item.issuedDate),
//       ReturnedDate: formatDate(item.returnedDate),
//     }));
...
const ViewReturnedAssets = () => {
  ...
  const handleRepairAction = async (assetReturnId, action) => {
    if (!action) return;
    try {
      await axios.patch(`http://localhost:5000/api/returns/repair-status/${assetReturnId}`, {
        action,
      });
      ...
      alert('Failed to update asset repair status.');
    }
  };
    ...
    <div>
      ...
      <table border="1" cellPadding="10" cellSpacing="0" style={{ width: '100%', textAlign: 'left' }}>
        ...
        <tbody>
          {filteredReturns.length === 0 ? (
            <tr>
              <td colSpan="12" style={{ textAlign: 'center' }}>No returned assets found</td>
            </tr>
          ) : (
            filteredReturns.map((item) => (
              <tr key={item._id}>
...
<<<<<<< HEAD
                <td>
                  <select onChange={(e) => handleRepairAction(item._id, e.target.value)} defaultValue="">
                    ...
                    <option value="restore">Restored to Stock</option>
